"""
Dinner Bot - 토양체질 Meal Recommender
...
**목표:**
퇴근 후 배달음식 대신 직접 요리할 수 있는 건강한 저녁 메뉴 **2가지**를 추천하세요.
**단백질 + 면역력 강화**에 초점을 맞추고, 토양체질에 최적화된 식재료만 사용하세요.

**필수 규칙:**
1. **딱 2가지 메뉴**만 추천하세요.
   - **첫 번째:** 가장 간단하고 쉬운 요리 (난이도 ★☆☆☆☆)
   - **두 번째:** 정성이 들어가는 별미 요리 (난이도 ★★★★★)
2. 토양체질 ✕✕(절대 금지) 식품은 어떤 메뉴에도 절대 포함하지 마세요
3. 토양체질 ✕(나쁨) 식품도 가급적 제외
4. ◎(매우 좋음)과 ○(좋음) 식품 위주로 구성
5. 각 메뉴는 서로 다른 주재료 사용
6. 실제로 맛있고 실용적인 레시피
7. 유튜브 영상은 **반드시** 아래 형식의 검색 결과 URL로 제공 (특정 동영상 ID 절대 금지)
   형식: `https://www.youtube.com/results?search_query=[메뉴이름]+레시피`
...
📺 *참고 영상:*
[메뉴이름 레시피](https://www.youtube.com/results?search_query=메뉴이름+레시피)
...
💡 *오늘의 추천:* (2개 중 유저의 상황에 맞춰 하나를 골라주세요)

위 형식으로 2가지 메뉴를 생성해주세요."""
    ...
    async def generate_leftover_menu(self, ingredients: str) -> str:
        """
        잔반 재료를 활용한 토양체질 맞춤 저녁 식단 2가지를 생성합니다.

        Args:
            ingredients: 사용자가 입력한 냉장고 잔반 재료 목록
        """
        prompt = f"""당신은 8체질 의학에 정통한 영양 전문가이자 요리 연구가입니다.
아래 '토양체질 식생표'를 **절대적으로** 준수하여, 사용자가 제공한 **잔반 재료**를 활용한 저녁 식단을 추천하세요.
...
**목표:**
사용자의 **잔반 재료**를 메인으로 활용하여, 토양체질에 맞는 건강한 저녁 메뉴 **2가지**를 추천하세요.
단, 제공된 재료 중 토양체질에 해로운 재료가 있다면 **과감히 제외하거나 대체**하고, 그 이유를 설명하세요.

**필수 규칙:**
1. **딱 2가지 메뉴**만 추천하세요.
   - **첫 번째:** 가장 간단하고 쉬운 요리 (난이도 ★☆☆☆☆)
   - **두 번째:** 정성이 들어가는 별미 요리 (난이도 ★★★★★)
2. **제공된 잔반 재료**를 최대한 활용하되, 토양체질에 맞지 않는다면 사용하지 마세요.
3. 잔반 재료 외에 필요한 추가 재료가 있다면 포함해도 됩니다.
4. 토양체질 ✕✕(절대 금지) 식품은 어떤 메뉴에도 절대 포함하지 마세요
5. 토양체질 ✕(나쁨) 식품도 가급적 제외
6. ◎(매우 좋음)과 ○(좋음) 식품 위주로 구성
7. 유튜브 영상은 **반드시** 아래 형식의 검색 결과 URL로 제공 (특정 동영상 ID 절대 금지)
   형식: `https://www.youtube.com/results?search_query=[메뉴이름]+레시피`
   예시: `https://www.youtube.com/results?search_query=돼지고기+된장찌개+레시피`
...
위 형식으로 2가지 메뉴를 생성해주세요."""
        ...
        try:
            ...
            logger.success(f"🍽️ 잔반 활용 식단 추천 생성 완료 (재료: {ingredients})")
            return result

        except Exception as e:
            error_msg = str(e)
            logger.error(f"❌ 잔반 식단 생성 실패: {error_msg}")
            return (
                "⚠️ 잔반 식단 생성 중 오류가 발생했습니다!\n\n"
                f"에러: {error_msg}\n\n"
                "잠시 후 다시 시도합니다."
            )
